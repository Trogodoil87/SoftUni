function solve(input) {
    ...
    for (let line of input) {
        let [name, firstArg, skill, secondArg, power] = line.split(' ');
        power = Number(power);

        if (firstArg === '->') {
            if (!gladis.hasOwnProperty(name)) {
                gladis[name] = {};
            }

            if (!gladis[name].hasOwnProperty(skill) || gladis[name][skill] < power) {
                gladis[name][skill] = power;
            }
        } else if (firstArg === 'vs') {
            let [first, second] = line.split(' vs ');
            if (gladis.hasOwnProperty(first) && gladis.hasOwnProperty(second)) {
                let firstTechSkills = gladis[first];
                let secondTechSkills = gladis[second];

                for (let skill in firstTechSkills) {
                    if (secondTechSkills.hasOwnProperty(skill)) {
                        if (firstTechSkills[skill] > secondTechSkills[skill]) {
                            delete gladis[second];
                        } else if (firstTechSkills[skill] < secondTechSkills[skill]) {
                            delete gladis[first];
                        }
                    }
                }
            }
        } else if (firstArg === 'Cesar') {
            break;
        }
    }
    for (let key in gladis) {
        let sum = 0;
        let valuesOfKey = gladis[key];
        for (let value in valuesOfKey) {
            sum += valuesOfKey[value];
        }

        valuesOfKey['sum'] = sum;
    }

    Object.entries(gladis)
